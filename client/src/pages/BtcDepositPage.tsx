...
export default function BtcDepositPage() {
  ...
  return <AppShell><div className="page-shell feature-shell btc-deposit-shell">
    <div className="workspace-heading feature-heading"><div><div className="breadcrumb"><span>Wallet</span><span>/</span><strong>Deposit ETH</strong></div><h1>Deposit Ethereum</h1><p>Connect your wallet and send ETH to the address shown below. This page uses your connected Ethereum wallet as the receiving address.</p></div><div className={network === "testnet" ? "network-badge testnet" : "network-badge"}>{network === "testnet" ? "TESTNET MODE" : "ETHEREUM MAINNET"}</div></div>
    {authLoading ? <section className="section-card settings-auth-card"><ShieldCheck size={32} /><h2>Checking your account</h2><p>Verifying your secure session before loading your Ethereum deposit details.</p></section> : requiresLogin ? <section className="section-card settings-auth-card"><ShieldCheck size={32} /><h2>Sign in to receive ETH</h2><p>Your wallet and deposit activity stay tied to your authenticated account.</p><Button type="button" onClick={() => setAuthOpen(true)}>Sign in</Button></section> : <>
      ...
      <div className="btc-deposit-grid">
        <section className="section-card btc-address-card">
          ...
          {walletLoading ? <div className="btc-loading"><Loader2 className="spin" size={18} /> Connecting wallet...</div> : connectedWallet ? <>
            ...
            <div className="wallet-empty-state">
              ...
              <p>Connect your wallet first. The ETH deposit address is derived from the wallet you connect here.</p>
            </div>
        ...
        <section className="section-card btc-address-card"><div className="section-header"><div><span className="section-eyebrow">Your deposit address</span><h2>Ethereum network</h2></div><span className="network-chip">{network}</span></div>{!depositAddress ? <div className="btc-provider-error">Connect a wallet to generate the ETH receiving address.</div> : <><div className="btc-qr-wrap">{qr && <img src={qr} alt="QR code for Ethereum deposit address" />}</div><div className="btc-address-box"><code>{depositAddress}</code><button type="button" aria-label="Copy Ethereum deposit address" onClick={copyAddress}><Copy size={16} /></button></div><p className="btc-warning">Only send ETH on the Ethereum network to this address. Sending other assets or using another network may result in permanent loss. This wallet address is your Ethereum deposit address for the account.</p></>}</section>
      </div>
