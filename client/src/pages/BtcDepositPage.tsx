...
export default function BtcDepositPage() {
  ...
  return <AppShell><div className="page-shell feature-shell btc-deposit-shell">
    ...
    {authLoading ? <section className="section-card settings-auth-card"><ShieldCheck size={32} /><h2>Checking your account</h2><p>Verifying your secure session before loading your Ethereum deposit details.</p></section> : requiresLogin ? <section className="section-card settings-auth-card"><ShieldCheck size={32} /><h2>Sign in to receive ETH</h2><p>Your deposit address and transaction history are private to your authenticated account.</p><Button type="button" onClick={() => setAuthOpen(true)}>Sign in</Button></section> : <>
      ...
      <aside className="section-card btc-info-card"><span className="section-eyebrow">Deposit details</span><div><span>Minimum deposit</span><strong>0.0001 ETH</strong></div><div><span>Required confirmations</span><strong>{addressQuery.data?.requiredConfirmations ?? 3}</strong></div><div><span>Estimated confirmation time</span><strong>1-5 minutes</strong></div><div><span>Daily yield</span><strong>12% APR auto-added every day</strong></div><div><span>Accounting</span><strong>Credited after confirmation</strong></div><a href="/deposits" className="button button-secondary button-sm">View deposit history <ExternalLink size={14} /></a><a href="/withdraw" className="button button-secondary button-sm" style={{ marginTop: "10px" }}>Withdraw savings <ExternalLink size={14} /></a></aside>
    </>}
