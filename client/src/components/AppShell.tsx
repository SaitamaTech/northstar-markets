      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <nav className="desktop-nav" aria-label="Primary navigation">
            ...
            <div className="more-nav-wrap"><button type="button" className="nav-link more-link" onClick={() => setMoreOpen(open => !open)} aria-expanded={moreOpen}><span>More</span><ChevronDown size={13} /></button>{moreOpen && <div className="more-menu"><Link href="/watchlist" onClick={() => setMoreOpen(false)}>Watchlists</Link><Link href="/portfolio" onClick={() => setMoreOpen(false)}>Portfolio</Link><Link href="/investments" onClick={() => setMoreOpen(false)}>Investments</Link><Link href="/investments/my-investments" onClick={() => setMoreOpen(false)}>My investments</Link><Link href="/calendar" onClick={() => setMoreOpen(false)}>Calendar</Link><Link href="/deposit/eth" onClick={() => setMoreOpen(false)}>Deposit ETH</Link><Link href="/deposits" onClick={() => setMoreOpen(false)}>Deposit history</Link>{user && <Link href="/settings" onClick={() => setMoreOpen(false)}><Settings size={13} /> Settings</Link>}</div>}</div>
          </nav>
